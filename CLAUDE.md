...
Appen collect-web 音频质检任务的自动化流水线：抢任务 → 抓题 → 调讯飞 ISE 评判 → 在页面批量点通过/拒绝。

实测 203 道任务总耗时约 **10 分钟**（collect 1' + evaluate 3' + mark 6'）。

## 三个 Skill 流水线

按顺序跑，每步通过 `result/[<workflow>/]<编号>.json` 串联：
...
## 抢任务（流水线第 0 步）：tools/grab_task.py

多个工作流（job）并行抢。脚本内置 `WORKFLOWS` 列表，不传 `--url` 默认轮询全部：

| code  | name | 备注                |
| ----- | ---- | ----------------- |
| A3189 | 中英信实 | 6000 小时同传 质检 1     |
| A3244 | 中英中哈 | 6000 小时同传 质检 1     |
| A3245 | 中英莓树 | 6000 小时同传 质检 1     |

```bash
source venv/bin/activate
python tools/grab_task.py                      # 轮询全部预置工作流
python tools/grab_task.py --only A3244,A3245    # 只轮指定 code
python tools/grab_task.py --url "https://.../qa?flowId=...&jobId=..."  # 临时单个
```

- **原理**：每个工作流占一个常驻 Playwright tab，页内 `fetch('/api-gw/collect/my-job/execute', {body:{jobId,flowId}})` 高频领取。execute 只认 body 里的 jobId/flowId，**与页面 URL 无关**（但每 tab 仍导航到自己工作流页面以保持 Referer 一致）。
- **多 tab 错峰轮换**：tab i 在 `i*interval` 首发，全局形成 A→B→C→A…。interval=0.2、3 工作流时 0.2s 试 A、0.4s 试 B、0.6s 试 C，每个工作流每 0.6s 一次。`--tabs` 默认 `max(2, 工作流数)`。
- **抢到即停**：任一返回 `data!=null`＝已领取（锁定到账号）→ 本机通知 + **钉钉**（`.env` 的 `DINGTALK_WEBHOOK`，消息含关键字「报告」过群机器人校验）→ 刷新进质检页 → 打印带 `--workflow` 的后续命令。
- **cookie 失效**（execute 返 401）也推钉钉，避免无人值守时静默挂掉。
- 服务器无 DISPLAY 自动 headless；root 下已加 `--no-sandbox`。`notify()` 在 Linux 缺 osascript/say 时静默跳过。

**my-job 接口**：`my-job/get`（只读，看 `remainQuantity`）；`my-job/execute`（POST `{jobId,flowId}`，无任务→`{"status":100103,"data":null}`，有任务→`data` 非空＝已领取）。

## 文件结构

```
.
├── .env                      # 讯飞凭据 + DINGTALK_WEBHOOK（.env.example 是占位模板，真值不入库）
├── bootstrap.sh              # Rocky Linux 9.6 一键初始化（系统依赖 + venv + .env），见文末
├── watch_jobs.sh             # 轮询 my-job/get 看新任务 + 解析 JWT 过期（也是 Python 读 cookie 的来源）
│                             #   COOKIE 入库是占位符；本地用 git skip-worktree 保留真值不被跟踪
...
├── qa_pipeline.py            # evaluate-tasks 主入口（process_one / result_path / cache_key）
├── tools/
│   ├── grab_task.py               # 抢任务：多工作流轮换领取（流水线第 0 步）
│   ├── collect_via_playwright.py  # collect-tasks-fast 主脚本
│   └── save_batch.py              # collect-tasks 慢路径用
├── result/[<workflow>/]<编号>.json # source of truth；编号跨工作流可能重复，故按工作流分目录
└── audio_cache/              # ffmpeg 转码的 .wav/.pcm（名 <workflow>_<编号>_qN），可随时清空
```
...
{
  "code": "6",
  "workflow": "A3244",        // 有则结果存 result/A3244/6.json；无则 flat result/6.json
  "totalQuestions": 203,
  "taskId": "...", "flowId": "...",
  "summary": {
    "totalQuestions": 203,
...
source venv/bin/activate
python tools/grab_task.py                       # 轮询全部预置工作流
python tools/grab_task.py --only A3244           # 只抢某个

# 2. collect-tasks-fast（Playwright，~1 分钟）；--workflow 决定 result/<workflow>/<编号>.json
python tools/collect_via_playwright.py 6 --workflow A3244 \
  --url "https://collect-web.appen.com.cn/collect/qa?flowId=...&jobId=...&taskId=..." \
  --interval 200

# 3. evaluate-tasks（并发 10，~3 分钟）
python qa_pipeline.py --code 6 --workflow A3244 --from-result --concurrency 10

# 4. mark-tasks（用 Chrome MCP，~6 分钟）
# AI 通过 Chrome MCP 在真实 Chrome 操作，读 result/<workflow>/<编号>.json（无 workflow 则 result/<编号>.json）
# 见 .claude/skills/mark-tasks/SKILL.md

# 5. 查看 result 状态（多工作流时路径为 result/<workflow>/<编号>.json）
python -c "
import json
from collections import Counter
doc = json.load(open('result/A3244/6.json'))
qs = doc['questions']
...
- `6` = 编号（页面左下"编号：N"）；`--workflow A3244` → 结果写 `result/A3244/6.json`（不传则 flat `result/6.json`）
...
- `--from-result` 直接读 `result/A3244/6.json`（不传 --workflow 则 `result/6.json`），跑所有 `status=='pending'` 的题
...
import json
from collections import Counter
doc = json.load(open('result/A3244/6.json'))   # 无 workflow 则 result/6.json
qs = doc['questions']
print('status:', dict(Counter(q['status'] for q in qs)))
print('summary:', doc['summary'])
print()
...
> 用 mark-tasks 根据 result/A3244/6.json 在页面 https://... 标注
...
## 部署到华为云竞价实例：bootstrap.sh

竞价实例随时可能被释放，新实例上一条命令拉起整个环境：
...
`bootstrap.sh`（面向 Rocky Linux 9.6 / RHEL 9，需 root 或 sudo）：
- EPEL + CRB 源 → 装 python3.11、chromium 运行库、ffmpeg（dnf 装不上则下静态构建）
- 建 venv + `pip install -r requirements.txt` + `playwright install chromium`
- 交互初始化 `.env`（讯飞凭据 + `DINGTALK_WEBHOOK`）

clone 后还要两件事才能跑：

1. `watch_jobs.sh` 的 `COOKIE` 是占位符 → copy as cURL 贴一份真 cookie（sliding session 会过期，常换）
2. grab_task / collect 在服务器自动 headless；**mark-tasks 跑不到服务器**（要真实 Chrome + Chrome MCP，在 Mac 上做）

**本地 cookie 与 git**：`watch_jobs.sh` 已 `git update-index --skip-worktree`，本地改 cookie 不被跟踪、不会误提交真 token。若要改脚本逻辑并提交，先 `git update-index --no-skip-worktree watch_jobs.sh`。
